...
public class BcSmartspaceCard extends LinearLayout {
    ...
    public void setSmartspaceTarget(SmartspaceTarget smartspaceTarget, BcSmartspaceCardLoggingInfo bcSmartspaceCardLoggingInfo, boolean z) {
        ...
        if (mDateView != null) {
            ...
            BcSmartSpaceUtil.setOnClickListener(mDateView, smartspaceTarget, new SmartspaceAction.Builder(uuid, "unusedTitle").setIntent(BcSmartSpaceUtil.getOpenCalendarIntent()).build(), "BcSmartspaceCard", mEventNotifier, bcSmartspaceCardLoggingInfo);
        }
    ...
    public void setNextAlarm(Drawable drawable, String str, SmartspaceTarget smartspaceTarget) {
        if (mNextAlarmImageView == null || mNextAlarmTextView == null) {
            return;
        }
        if (drawable == null) {
            mNextAlarmImageView.setVisibility(View.GONE);
    ...
    private void setNextAlarmClickListener(View view, SmartspaceTarget smartspaceTarget) {
        ...
        if (smartspaceTarget == null) {
            build = new BcSmartspaceCardLoggingInfo.Builder().setInstanceId(InstanceId.create("upcoming_alarm_card_94510_12684")).setFeatureType(23).setDisplaySurface(BcSmartSpaceUtil.getLoggingDisplaySurface(getContext().getPackageName(), mDozeAmount)).build();
        } else {
            build = new BcSmartspaceCardLoggingInfo.Builder().setInstanceId(InstanceId.create(smartspaceTarget)).setFeatureType(smartspaceTarget.getFeatureType()).setDisplaySurface(BcSmartSpaceUtil.getLoggingDisplaySurface(getContext().getPackageName(), mDozeAmount)).build();
        }
        BcSmartSpaceUtil.setOnClickListener(view, smartspaceTarget, SHOW_ALARMS_ACTION, "BcSmartspaceCard", mEventNotifier, build);
    }
