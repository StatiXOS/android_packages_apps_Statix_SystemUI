...
public class BcSmartspaceView extends FrameLayout implements BcSmartspaceDataPlugin.SmartspaceTargetListener, BcSmartspaceDataPlugin.SmartspaceView {
    ...
    private final ViewPager.OnPageChangeListener mOnPageChangeListener = new ViewPager.OnPageChangeListener() { // from class: com.google.android.systemui.smartspace.BcSmartspaceView.2
        ...
        public void onPageSelected(int i) {
            ...
            mCurrentPosition = i;
            SmartspaceTarget targetAtPosition = mAdapter.getTargetAtPosition(mCurrentPosition);
            logSmartspaceEvent(targetAtPosition, mCurrentPosition, BcSmartspaceEvent.SMARTSPACE_CARD_SEEN);
    ...
    public void logCurrentDisplayedCardSeen() {
        ...
        if (targetAtPosition == null) {
            ...
            logSmartspaceEvent(targetAtPosition, mCardPosition, BcSmartspaceEvent.SMARTSPACE_CARD_SEEN);
        }
    ...
    public void onSmartspaceTargetsUpdated(List<? extends Parcelable> list) {
        ...
        mLastReceivedTargets.addAll((Collection) mAdapter.getTargets().stream().map(smartspaceTarget -> smartspaceTarget.getSmartspaceTargetId()).collect(Collectors.toList()));
    ...
    public void logSmartspaceEvent(SmartspaceTarget smartspaceTarget, int i, BcSmartspaceEvent bcSmartspaceEvent) {
        ...
        BcSmartspaceCardLoggingInfo build = new BcSmartspaceCardLoggingInfo.Builder().setInstanceId(InstanceId.create(smartspaceTarget)).setFeatureType(smartspaceTarget.getFeatureType()).setDisplaySurface(BcSmartSpaceUtil.getLoggingDisplaySurface(getContext().getPackageName(), mAdapter.getDozeAmount())).setRank(i).setCardinality(mAdapter.getCount()).setReceivedLatency(i2).setSubcardInfo(BcSmartspaceCardLoggerUtil.createSubcardLoggingInfo(smartspaceTarget)).build();
    ...
    private void animateSmartspaceUpdate(final BcSmartspaceCard bcSmartspaceCard) {
        if (bcSmartspaceCard != null && mRunningAnimation == null && bcSmartspaceCard.getParent() == null) {
            final ViewGroup viewGroup = (ViewGroup) mViewPager.getParent();
            bcSmartspaceCard.measure(View.MeasureSpec.makeMeasureSpec(mViewPager.getWidth(), 1073741824), View.MeasureSpec.makeMeasureSpec(mViewPager.getHeight(), 1073741824));
            bcSmartspaceCard.layout(mViewPager.getLeft(), mViewPager.getTop(), mViewPager.getRight(), mViewPager.getBottom());
    ...
    public void setDozeAmount(float f) {
        ...
        int loggingDisplaySurface = BcSmartSpaceUtil.getLoggingDisplaySurface(getContext().getPackageName(), mAdapter.getDozeAmount());
