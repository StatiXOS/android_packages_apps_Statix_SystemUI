...
public class BcSmartspaceCardLoggerUtil {
    ...
    public static void forcePrimaryFeatureTypeAndInjectWeatherSubcard(BcSmartspaceCardLoggingInfo bcSmartspaceCardLoggingInfo, SmartspaceTarget smartspaceTarget, int i) {
        if (bcSmartspaceCardLoggingInfo == null || bcSmartspaceCardLoggingInfo.getFeatureType() != 1) {
            return;
        }
        ...
        if (bcSmartspaceCardLoggingInfo.getSubcardInfo() == null) {
            bcSmartspaceCardLoggingInfo.setSubcardInfo(new BcSmartspaceSubcardLoggingInfo.Builder().setClickedSubcardIndex(0).setSubcards(new ArrayList()).build());
        }
        ...
        bcSmartspaceCardLoggingInfo.getSubcardInfo().getSubcards().add(new BcSmartspaceCardMetadataLoggingInfo.Builder().setInstanceId(InstanceId.create(smartspaceTarget)).setCardTypeId(1).build());
    }
    ...
    public static BcSmartspaceSubcardLoggingInfo createSubcardLoggingInfo(SmartspaceTarget smartspaceTarget) {
        ...
        BcSmartspaceCardMetadataLoggingInfo build = new BcSmartspaceCardMetadataLoggingInfo.Builder().setInstanceId(create).setCardTypeId(baseAction.getExtras().getInt("subcardType")).build();
        ArrayList arrayList = new ArrayList();
        arrayList.add(build);
        return new BcSmartspaceSubcardLoggingInfo.Builder().setSubcards(arrayList).setClickedSubcardIndex(0).build();
    }
}
