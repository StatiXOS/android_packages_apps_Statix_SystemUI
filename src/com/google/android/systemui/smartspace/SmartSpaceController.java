...
public class SmartSpaceController implements Dumpable {
    ...
    public SmartSpaceController(Context context, KeyguardUpdateMonitor keyguardUpdateMonitor, Handler handler, AlarmManager alarmManager, DumpManager dumpManager) {
        KeyguardUpdateMonitorCallback keyguardUpdateMonitorCallback = new KeyguardUpdateMonitorCallback() {
            @Override
                        ...
                        mUiHandler.post(new Runnable() {
                            ...
                            public final void run() {
                                if (newCardInfo.isPrimary()) {
                                    mData.mCurrentCard = fromWrapper;
                                } else {
                                    mData.mWeatherCard = fromWrapper;
                                }
                                mData.handleExpire();
                                update();
                            }
    ...
    private void clearStore() {
        mStore.store(null, "smartspace_" + mCurrentUserId + "_true");
        mStore.store(null, "smartspace_" + mCurrentUserId + "_false");
    }
