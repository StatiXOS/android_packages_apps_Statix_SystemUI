...
public class KeyguardIndicationControllerStatix extends KeyguardIndicationController {
    ...
    public KeyguardIndicationControllerStatix(
        ...
        mAdaptiveChargingStatusReceiver = new AdaptiveChargingManager.AdaptiveChargingStatusReceiver() {
            ...
            public void onReceiveStatus(int seconds, String stage) {
                boolean isActiveOrEnabled = isStageActiveOrEnabled(stage);
                boolean shouldEnable = false;
                if (isActiveOrEnabled && seconds > 0) {
                    shouldEnable = true;
                }
                ...
                if (mAdaptiveChargingActive != shouldEnable  || (shouldEnable && abs > TimeUnit.SECONDS.toMillis(30L))) {
                    mAdaptiveChargingActive = shouldEnable;
    ...
    private static boolean isStageActiveOrEnabled(String stage) {
       return !"Active".equals(stage) && !"Enabled".equals(stage);
    }
    ...
    private void refreshAdaptiveChargingEnabled() {
        ...
        boolean supported = mAdaptiveChargingManager.hasAdaptiveChargingFeature() && DeviceConfig.getBoolean("adaptive_charging", "adaptive_charging_enabled", true);
        if (supported) {
            mAdaptiveChargingEnabledInSettings = Settings.Secure.getInt(mContext.getContentResolver(), "adaptive_charging_enabled", 1) == 1;
        } else {
    ...
    public final void init() {
        ...
        DeviceConfig.OnPropertiesChangedListener onPropertiesChangedListener = new DeviceConfig.OnPropertiesChangedListener() {
            @Override
        ...
        mDeviceConfig.addOnPropertiesChangedListener("adaptive_charging", delayableExecutor, onPropertiesChangedListener);
        triggerAdaptiveChargingStatusUpdate();
