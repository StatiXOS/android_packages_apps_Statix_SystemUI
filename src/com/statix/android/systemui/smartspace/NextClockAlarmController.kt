...
class NextClockAlarmController @Inject constructor(
    ...
    val userUnlockReceiver = object : BroadcastReceiver() {
    ...
    val userChangedCallback = object : UserTracker.Callback {
        override fun onBeforeUserSwitching(newUserId: Int) {
            ...
            alarmAppSearchController.searchSession?.let { session ->
                try {
                    session.unregisterObserverCallback("com.google.android.deskclock", observerCallback)
                } catch (e: Exception) {
        ...
        override fun onUserChanged(newUserId: Int, userContext: Context) {
            Log.d(TAG, "onUserChanged newUser=$newUserId, oldUser=${context?.userId}, userContext=$userContext")
            if (isUserUnlocked()) {
    ...
    val observerCallback = object : ObserverCallback {
        override fun onDocumentChanged(changeInfo: DocumentChangeInfo) {
            ...
            updateNextAlarmJob = applicationScope.launch {
                updateNextAlarm()
            }
        }
    ...
    fun updateSession(context: Context) {
        ...
        updateSessionJob = backgroundScope.launch {
    ...
    suspend fun calculateNextClockAlarm(searchResults: SearchResults?): Long = suspendCoroutine { continuation ->
        ...
        suspend fun processResults(results: List<SearchResult>) {
            for (result in results) {
                ...
                val daysOfWeek = document.getPropertyStringArray("daysOfWeek")?.joinToString(", ") ?: ""
                val id = document.id
                alarmInfo = "Alarm id=$id, nextTime=$scheduledTime, nextTimeInMillis=$timeInMillis, days=[$daysOfWeek]"
                Log.d(TAG, alarmInfo)
        ...
        applicationScope.launch {
            var results = searchResults?.let { alarmAppSearchController.getNextPageSearchResults(it) } ?: emptyList()
