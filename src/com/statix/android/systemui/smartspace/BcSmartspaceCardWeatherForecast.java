...
public class BcSmartspaceCardWeatherForecast extends BcSmartspaceCardSecondary {
    ...
    protected void onFinishInflate() {
        ...
        for (int i = 0; i < 4; i++) {
            ConstraintLayout column = (ConstraintLayout) View.inflate(getContext(), R.layout.smartspace_card_weather_forecast_column, null);
    ...
    public boolean setSmartspaceActions(SmartspaceTarget target, BcSmartspaceDataPlugin.SmartspaceEventNotifier eventNotifier, BcSmartspaceCardLoggingInfo loggingInfo) {
        ...
        if (extras.containsKey("temperatureValues")) {
            ...
            if (temperatureValues == null) {
                ...
                updateFields((view, index) -> ((TextView) view).setText(temperatureValues[index]),
        ...
        if (extras.containsKey("weatherIcons")) {
            ...
            if (weatherIcons == null) {
                ...
                updateFields((view, index) -> ((ImageView) view).setImageBitmap(weatherIcons[index]),
        ...
        if (extras.containsKey("timestamps")) {
            ...
            if (timestamps == null) {
                ...
                updateFields((view, index) -> ((TextView) view).setText(timestamps[index]),
    ...
    private void updateFields(ItemUpdateFunction updateFunction, int count, int viewId, String fieldName) {
        ...
        if (childCount < 4) {
            Log.w("BcSmartspaceCardWeatherForecast",
                    String.format(Locale.US, "Missing %d %s view(s) to update.", 4 - childCount, fieldName));
            return;
        }
        ...
        if (count < 4) {
            Log.w("BcSmartspaceCardWeatherForecast",
                    String.format(Locale.US, "Missing %d %s(s). Hiding incomplete columns.", 4 - count, fieldName));
            for (int i = 0; i < 4; i++) {
                View column = getChildAt(i);
                BcSmartspaceTemplateDataUtils.updateVisibility(column, i <= (3 - (4 - count)) ? View.VISIBLE : View.GONE);
            }
        ...
        for (int i = 0; i < columnCount; i++) {
            ...
            if (targetView == null) {
                Log.w("BcSmartspaceCardWeatherForecast",
                        String.format(Locale.US, "Missing %s view to update at column: %d.", fieldName, i + 1));
                return;
            }
