...
public class CardPagerAdapter extends PagerAdapter implements CardAdapter {
    public static class Companion {
        ...
        public boolean useRecycledViewForAction(SmartspaceAction newAction, SmartspaceAction recycledAction) {
        ...
        public boolean useRecycledViewForActionsList(List<SmartspaceAction> newActions, List<SmartspaceAction> recycledActions) {
        ...
        public boolean useRecycledViewForNewTarget(SmartspaceTarget newTarget, SmartspaceTarget recycledTarget) {
            ...
            if (!useRecycledViewForAction(newTarget.getHeaderAction(), recycledTarget.getHeaderAction())) {
                return false;
            }
            if (!useRecycledViewForAction(newTarget.getBaseAction(), recycledTarget.getBaseAction())) {
                return false;
            }
            if (!useRecycledViewForActionsList(newTarget.getActionChips(), recycledTarget.getActionChips())) {
                return false;
            }
            ...
            return newTemplateData != null && recycledTemplateData != null && newTemplateData.equals(recycledTemplateData);
        }
    }
    ...
    private final LazyServerFlagLoader enableCardRecycling = new LazyServerFlagLoader("enable_card_recycling");
    private final LazyServerFlagLoader enableReducedCardRecycling = new LazyServerFlagLoader("enable_reduced_card_recycling");
    ...
    private final SparseArray<BcSmartspaceRemoteViewsCard> recycledRemoteViewsCards = new SparseArray<>();
    ...
    public void destroyItem(ViewGroup container, int position, Object object) {
        ...
        if (holder.legacyCard != null && holder.target != null && enableCardRecycling.get()) {
            recycledLegacyCards.put(BcSmartSpaceUtil.getFeatureType(holder.target), holder.legacyCard);
        }
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        ...
        if (remoteViews != null) {
            ...
            BaseTemplateCard templateCard = enableCardRecycling.get() ? recycledCards.removeReturnOld(target.getFeatureType()) : null;
            ...
            if (templateCard == null) {
                //STX comment-out // Always use smartspace_base_template_card to avoid date view in secondary cards
                //STX comment-out int layoutRes = R.layout.smartspace_base_template_card;
                BaseTemplateData.SubItemInfo primaryItem = newTemplateData != null ? newTemplateData.getPrimaryItem() : null; //STX edit
                int layoutRes = (primaryItem == null || (SmartspaceUtils.isEmpty(primaryItem.getText()) && primaryItem.getIcon() == null)) ? R.layout.smartspace_base_template_card_with_date : R.layout.smartspace_base_template_card; //STX edit
                templateCard = (BaseTemplateCard) LayoutInflater.from(container.getContext()).inflate(layoutRes, container, false);
                ...
                if (nonRemoteViewsHorizontalPadding != null) {
                    templateCard.setPaddingRelative(nonRemoteViewsHorizontalPadding, templateCard.getPaddingTop(),
                            nonRemoteViewsHorizontalPadding, templateCard.getPaddingBottom());
                }
                ...
                Map<Integer, Integer> templateTypeToSecondaryCardRes = BcSmartspaceTemplateDataUtils.TEMPLATE_TYPE_TO_SECONDARY_CARD_RES;
                Integer secondaryRes = templateTypeToSecondaryCardRes.get(target.getTemplateData().getTemplateType());
                if (secondaryRes != null) {
                    BcSmartspaceCardSecondary secondaryCard = (BcSmartspaceCardSecondary) LayoutInflater.from(container.getContext())
            ...
            if (legacyCard == null) {
                ...
                if (layoutRes == 0) {
                    Log.w("SsCardPagerAdapter", "No legacy card can be created for feature type: " + featureType);
                } else {
                    legacyCard = (BcSmartspaceCard) LayoutInflater.from(container.getContext()).inflate(layoutRes, container, false);
                    legacyCard.mUiSurface = uiSurface;
                    if (nonRemoteViewsHorizontalPadding != null) {
                        legacyCard.setPaddingRelative(nonRemoteViewsHorizontalPadding, legacyCard.getPaddingTop(),
                                nonRemoteViewsHorizontalPadding, legacyCard.getPaddingBottom());
                    }
    ...
    public int getItemPosition(Object object) {
        ...
        if (currentTarget != null &&
                BcSmartSpaceUtil.getFeatureType(currentTarget) == BcSmartSpaceUtil.getFeatureType(holder.target) &&
    ...
    public void onBindViewHolder(ViewHolder holder) {
        ...
        BcSmartspaceCardLoggingInfo.Builder loggingInfoBuilder = new BcSmartspaceCardLoggingInfo.Builder()
                ...
                .setUid(-1);
        BcSmartspaceSubcardLoggingInfo subcardInfo = hasValidTemplate
        ...
        loggingInfoBuilder.setDimensionalInfo(BcSmartspaceCardLoggerUtil.createDimensionalLoggingInfo(target.getTemplateData()));
        ...
        if (target.getRemoteViews() != null) {
            ...
            BcSmartspaceDataPlugin.SmartspaceEventNotifier notifier = dataProvider != null ? event -> dataProvider.notifySmartspaceEvent(event) : null;
            holder.remoteViewsCard.bindData(target, notifier, loggingInfo, smartspaceTargets.size() > 1);
            ...
            BcSmartspaceCardLoggerUtil.tryForcePrimaryFeatureTypeAndInjectWeatherSubcard(loggingInfo, target);
            BcSmartspaceDataPlugin.SmartspaceEventNotifier notifier = dataProvider != null ? event -> dataProvider.notifySmartspaceEvent(event) : null;
    ...
    public void setDozeAmount(float dozeAmount) {
        ...
        TransitionType newTransition = previousDozeAmount > dozeAmount ? TransitionType.TO_LOCKSCREEN :
                previousDozeAmount < dozeAmount ? TransitionType.TO_AOD : TransitionType.NOT_IN_TRANSITION;
    ...
    public void setTargets(List<SmartspaceTarget> targets) {
        ...
        for (SmartspaceTarget target : targets) {
            ...
            int screenExtra = target.getBaseAction() != null && target.getBaseAction().getExtras() != null
    ...
    public void updateTargetVisibility() {
        List<SmartspaceTarget> targetList = !mediaTargets.isEmpty() ? mediaTargets :
                hasDifferentTargets ? _aodTargets : getLockscreenTargets();
        List<SmartspaceTarget> lockscreenTargets = getLockscreenTargets();
        boolean shouldUpdate = smartspaceTargets != targetList && (dozeAmount == 1f || (dozeAmount >= 0.36f && transitioningTo == TransitionType.TO_AOD));
        boolean shouldUpdateLockscreen = smartspaceTargets != lockscreenTargets && (dozeAmount == 0f || (1f - dozeAmount >= 0.36f && transitioningTo == TransitionType.TO_LOCKSCREEN));
