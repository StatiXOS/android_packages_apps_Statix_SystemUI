...
class KeyguardZenAlarmViewController @Inject constructor(
    ...
    val showNextAlarm = object : AlarmManager.OnAlarmListener {
        override fun onAlarm() {
            showAlarm()
        }
    }

    val nextAlarmCallback = object : NextAlarmCallback {
    ...
    init {
        val dndDrawable = context.resources.getDrawable(R.drawable.stat_sys_dnd, null) as? Drawable
    ...
    fun showAlarm(): Job = applicationScope.launch {
        ...
        if (nextAlarmTime > 0) {
            ...
            if (nextAlarmTime <= twelveHoursLater) {
                ...
                val formattedTime = android.text.format.DateFormat.format(format, nextAlarmTime).toString()
                smartspaceViews.forEach { view ->
                    view.setNextAlarm(alarmImage, formattedTime)
                }
            } else {
                smartspaceViews.forEach { view ->
    ...
    fun updateModeIcon(view: BcSmartspaceDataPlugin.SmartspaceView, zenModeInfo: ZenModeInfo?) {
        applicationScope.launch {
            if (zenModeInfo != null) {
                val drawable = zenModeInfo.icon.drawable.let { d ->
                    d.constantState?.newDrawable(context.resources) ?: d.mutate()
                }
                val description = context.getString(R.string.active_mode_content_description, zenModeInfo.name)
