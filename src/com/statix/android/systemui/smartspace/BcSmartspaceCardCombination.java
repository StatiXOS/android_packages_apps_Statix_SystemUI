...
public class BcSmartspaceCardCombination extends BcSmartspaceCardSecondary {
    ...
    public boolean setSmartspaceActions(SmartspaceTarget target, BcSmartspaceDataPlugin.SmartspaceEventNotifier eventNotifier, BcSmartspaceCardLoggingInfo loggingInfo) {
        ...
        if (mFirstSubCard != null) {
            firstCardSet = fillSubCard(mFirstSubCard, target, firstAction, eventNotifier, loggingInfo);
        }
        ...
        if (secondCardAvailable) {
            ...
            secondCardSet = fillSubCard(mSecondSubCard, target, secondAction, eventNotifier, loggingInfo);
        }
    ...
    public boolean fillSubCard(ConstraintLayout subCard, SmartspaceTarget target, SmartspaceAction action, BcSmartspaceDataPlugin.SmartspaceEventNotifier eventNotifier, BcSmartspaceCardLoggingInfo loggingInfo) {
        ...
        BcSmartSpaceUtil.setOnClickListener(subCard, target, action, eventNotifier, "BcSmartspaceCardCombination", loggingInfo, 0);

        Drawable iconDrawable = action.getIcon() != null
                ? BcSmartSpaceUtil.getIconDrawableWithCustomSize(action.getIcon(), getContext(), getResources().getDimensionPixelSize(R.dimen.enhanced_smartspace_icon_size))
                : null;
