...
public class DateSmartspaceView extends LinearLayout implements BcSmartspaceDataPlugin.SmartspaceView {
    ...
    public DateSmartspaceView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mUiSurface = null;
        mDozeAmount = 0f;
        mDateTarget = new SmartspaceTarget.Builder(
                "date_card_794317_92634",
                new ComponentName(context, DateSmartspaceView.class),
                context.getUser())
                .setFeatureType(1)
                .build();
        mDateAction = new SmartspaceAction.Builder("dateId", "Date")
    ...
    protected void onAttachedToWindow() {
        ...
        BcSmartspaceDataPlugin.SmartspaceEventNotifier notifier = mDataProvider != null ? event -> mDataProvider.notifySmartspaceEvent(event) : null;
        BcSmartSpaceUtil.setOnClickListener(mDateView, mDateTarget, mDateAction, notifier, "DateSmartspaceView", mLoggingInfo, 0);
    }
    ...
    protected void onFinishInflate() {
        ...
        //STX comment-out BcSmartspaceTemplateDataUtils.updateVisibility(mDateView, View.VISIBLE); // Show date view for primary display
    }
    ...
    public void setDnd(Drawable drawable, String description) {
        new Handler(Looper.getMainLooper()).post(() -> { //STX edit
        if (drawable == null) {
            ...
            //STX comment-out BcSmartspaceTemplateDataUtils.updateVisibility(mDateView, View.GONE); // Hide date view when DND is present
        }
    ...
    public void setDozeAmount(float dozeAmount) {
        ...
        if (mNextAlarmData.mImage != null) {
            BcSmartspaceCardLoggingInfo.Builder alarmBuilder = new BcSmartspaceCardLoggingInfo.Builder()
                    .setInstanceId(InstanceId.create("upcoming_alarm_card_94510_12684"))
                    .setFeatureType(23)
                    .setDisplaySurface(loggingSurface)
                    .setUid(mLoggingInfo.mUid);
            BcSmartspaceCardLogger.log(BcSmartspaceEvent.SMARTSPACE_CARD_SEEN, new BcSmartspaceCardLoggingInfo(alarmBuilder));
        }
    }
    ...
    public void setNextAlarm(Drawable drawable, String description) {
        ...
        if (mNextAlarmData.mImage == null) {
            ...
            // STX comment-out BcSmartspaceTemplateDataUtils.updateVisibility(mDateView, View.VISIBLE); // Show date view when no alarm data
        } else {
            mNextAlarmTextView.setContentDescription(getContext().getString(R.string.accessibility_next_alarm, description));
            String displayText = TextUtils.isEmpty(null) ? description : description + " · null";
            ...
            //STX comment-out BcSmartspaceTemplateDataUtils.updateVisibility(mDateView, View.GONE); // Hide date view when alarm data is present
            BcSmartspaceDataPlugin.SmartspaceEventNotifier notifier = mDataProvider != null ? event -> mDataProvider.notifySmartspaceEvent(event) : null;
            int loggingSurface = BcSmartSpaceUtil.getLoggingDisplaySurface(mUiSurface, mDozeAmount);
            BcNextAlarmData.setOnClickListener(mNextAlarmTextView, notifier, loggingSurface);
        }
        updateColorForExtras();
        }); //STX edit
    }
    ...
    public void setUiSurface(String uiSurface) {
        if (!isAttachedToWindow()) {
            ...
            if (TextUtils.equals(uiSurface, "lockscreen") && mDateView != null && !mDateView.isAttachedToWindow()) {
