...
public class BcSmartspaceCardDoorbell extends BcSmartspaceCardGenericImage {
    ...
    public boolean setSmartspaceActions(SmartspaceTarget target, BcSmartspaceDataPlugin.SmartspaceEventNotifier notifier,
        ...
        List<String> imageUris = target.getIconGrid().stream()
        ...
        if (!imageUris.isEmpty()) {
            ...
            List<DrawableWithUri> drawables = uris.stream()
                    .map(uri -> {
                        DrawableWithUri drawable = mUriToDrawable.computeIfAbsent(uri, k ->
                                new DrawableWithUri(uri, contentResolver, height, cornerRadius, imageViewRef, loadingScreenRef));
            ...
            drawable.setCornerRadius(res.getDimension(R.dimen.enhanced_smartspace_secondary_card_corner_radius));
    ...
    public static class DrawableWithUri extends DrawableWrapper {
        ...
        public DrawableWithUri(Uri uri, ContentResolver contentResolver, int heightInPx, float scaledCornerRadius,
                              WeakReference<ImageView> imageViewRef, WeakReference<ViewGroup> loadingScreenRef) {
    ...
    public static class LoadUriTask extends AsyncTask<DrawableWithUri, Void, DrawableWithUri> {
        ...
        protected DrawableWithUri doInBackground(DrawableWithUri... params) {
            ...
            try (InputStream inputStream = drawable.mContentResolver.openInputStream(drawable.mUri)) {
                android.graphics.ImageDecoder.Source source = android.graphics.ImageDecoder.createSource(null, inputStream);
                drawable.mDrawable = android.graphics.ImageDecoder.decodeDrawable(source, (decoder, info, src) -> {
                    decoder.setAllocator(android.graphics.ImageDecoder.ALLOCATOR_SOFTWARE);
                    int height = drawable.mHeightInPx;
                    float aspectRatio = info.getSize().getHeight() != 0 ?
                            (float) info.getSize().getWidth() / info.getSize().getHeight() : 0;
                    int width = (int) (height * aspectRatio);
        ...
        protected void onPostExecute(DrawableWithUri result) {
            ...
            if (result.mDrawable != null) {
                ...
                if (result.mUri != null && mInstrumentContext.mUriSet.remove(result.mUri) && mInstrumentContext.mUriSet.isEmpty()) {
