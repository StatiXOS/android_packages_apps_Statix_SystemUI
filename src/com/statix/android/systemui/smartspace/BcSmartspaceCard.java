...
public class BcSmartspaceCard extends ConstraintLayout implements SmartspaceCard {
    ...
    @Override
    public void bindData(SmartspaceTarget target, BcSmartspaceDataPlugin.SmartspaceEventNotifier eventNotifier, BcSmartspaceCardLoggingInfo loggingInfo, boolean usePageIndicatorUi) {
        mLoggingInfo = null;
        mEventNotifier = null;
        ...
        if (headerAction != null) {
            if (mSecondaryCard != null) {
                ...
                mValidSecondaryCard = mSecondaryCard.setSmartspaceActions(target, eventNotifier, loggingInfo);
            }
            if (mSecondaryCardGroup != null) {
                mSecondaryCardGroup.setAlpha(1f);
                BcSmartspaceTemplateDataUtils.updateVisibility(mSecondaryCardGroup, mDozeAmount == 1f || !mValidSecondaryCard ? View.GONE : View.VISIBLE);
            }

            Icon icon = headerAction.getIcon();
            Drawable iconDrawable = icon != null ? BcSmartSpaceUtil.getIconDrawableWithCustomSize(icon, getContext(), getResources().getDimensionPixelSize(R.dimen.enhanced_smartspace_icon_size)) : null;
        ...
        if (mBaseActionIconSubtitleView != null && baseAction != null) {
            Drawable baseIconDrawable = baseAction.getIcon() != null
                    ? BcSmartSpaceUtil.getIconDrawableWithCustomSize(baseAction.getIcon(), getContext(), getResources().getDimensionPixelSize(R.dimen.enhanced_smartspace_icon_size))
                    : null;
            mBaseActionIconDrawable.setIcon(baseIconDrawable);

            if (baseIconDrawable == null) {
                BcSmartspaceTemplateDataUtils.updateVisibility(mBaseActionIconSubtitleView, View.INVISIBLE);
                mBaseActionIconSubtitleView.setOnClickListener(null);
                mBaseActionIconSubtitleView.setContentDescription(null);
            } else {
                mBaseActionIconSubtitleView.setText(baseAction.getSubtitle());
                mBaseActionIconSubtitleView.setCompoundDrawablesRelative(mBaseActionIconDrawable, null, null, null);
                BcSmartspaceTemplateDataUtils.updateVisibility(mBaseActionIconSubtitleView, View.VISIBLE);

                int subcardType = baseAction.getExtras() != null && !baseAction.getExtras().isEmpty()
                        ? baseAction.getExtras().getInt("subcardType", -1)
                        : -1;
                int clickedIndex = subcardType != -1 ? getClickedIndex(loggingInfo, subcardType) : 0;
                if (clickedIndex == 0) {
                    Log.d("BcSmartspaceCard", "Subcard expected but missing type. loggingInfo=" + loggingInfo + ", baseAction=" + baseAction);
                }
                BcSmartSpaceUtil.setOnClickListener(mBaseActionIconSubtitleView, target, baseAction, mEventNotifier, "BcSmartspaceCard", loggingInfo, clickedIndex);
                ContentDescriptionUtil.setFormattedContentDescription("BcSmartspaceCard", mBaseActionIconSubtitleView, baseAction.getSubtitle(), baseAction.getContentDescription());
            }
        }
        ...
        ConstraintLayout.LayoutParams params = (ConstraintLayout.LayoutParams) mSecondaryCardGroup.getLayoutParams();
        params.matchConstraintMaxWidth = BcSmartSpaceUtil.getFeatureType(target) == -2 ? getWidth() * 3 / 4 : getWidth() / 2;
    ...
    protected void onFinishInflate() {
        ...
        int paddingStart = getResources().getDimensionPixelSize(R.dimen.non_remoteviews_card_padding_start);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mBaseActionIconSubtitleView == null || mBaseActionIconSubtitleView.getVisibility() != View.VISIBLE) {
            return;
        }

        int padding = getResources().getDimensionPixelSize(R.dimen.subtitle_hit_rect_height) - mBaseActionIconSubtitleView.getHeight();
        padding = padding / 2;
        ...
        setTouchDelegate(new TouchDelegate(mBaseActionIconSubtitleHitRect, mBaseActionIconSubtitleView));
    }
    ...
    public void setDozeAmount(float dozeAmount) {
        ...
        int secondaryCardVisibility = (mDozeAmount == 1f || !mValidSecondaryCard) ? View.GONE : View.VISIBLE;
        BcSmartspaceTemplateDataUtils.updateVisibility(mSecondaryCardGroup, secondaryCardVisibility);

        if (mTarget != null && mTarget.getFeatureType() == 30) {
            return;
        }

        if (mSecondaryCardGroup != null && mSecondaryCardGroup.getVisibility() == View.VISIBLE && mTextGroup != null) {
            ...
            mTextGroup.setTranslationX(translation * ((PathInterpolator) interpolator).getInterpolation(mDozeAmount));
    ...
    public void setPrimaryOnClickListener(SmartspaceTarget target, SmartspaceAction action, BcSmartspaceCardLoggingInfo loggingInfo) {
        if (action != null) {
            BcSmartSpaceUtil.setOnClickListener(this, target, action, mEventNotifier, "BcSmartspaceCard", loggingInfo, 0);
            if (mTitleTextView != null) {
                BcSmartSpaceUtil.setOnClickListener(mTitleTextView, target, action, mEventNotifier, "BcSmartspaceCard", loggingInfo, 0);
            }
            if (mSubtitleTextView != null) {
                BcSmartSpaceUtil.setOnClickListener(mSubtitleTextView, target, action, mEventNotifier, "BcSmartspaceCard", loggingInfo, 0);
            }
        }
    }
    ...
    public void setSubtitle(CharSequence text, CharSequence contentDescription, boolean useIcon) {
        ...
        mSubtitleTextView.setCompoundDrawablesRelative(useIcon ? mIconDrawable : null, null, null, null);
        mSubtitleTextView.setMaxLines((mTarget != null && mTarget.getFeatureType() == 5 && !mUsePageIndicatorUi) ? 2 : 1);
    ...
    public void setTitle(CharSequence text, CharSequence contentDescription, boolean useIcon) {
        ...
        Bundle extras = (mTarget != null && mTarget.getHeaderAction() != null) ? mTarget.getHeaderAction().getExtras() : null;
        if (extras != null && extras.containsKey("titleEllipsize")) {
            try {
                ...
                Log.w("BcSmartspaceCard", "Invalid TruncateAt value: " + extras.getString("titleEllipsize"));
            }
        } else if (mTarget != null && mTarget.getFeatureType() == 2 && Locale.ENGLISH.getLanguage().equals(getResources().getConfiguration().locale.getLanguage())) {
        ...
        mTitleTextView.setCompoundDrawablesRelative(showIcon ? mIconDrawable : null, null, null, null);
        BcSmartspaceTemplateDataUtils.offsetTextViewForIcon(mTitleTextView, showIcon ? mIconDrawable : null, isRtl());
    }
    ...
    public void updateIconTint() {
        ...
        if (mBaseActionIconDrawable != null) {
            ...
            int subcardType = (baseAction != null && baseAction.getExtras() != null && !baseAction.getExtras().isEmpty())
                    ? baseAction.getExtras().getInt("subcardType", -1)
                    : -1;
