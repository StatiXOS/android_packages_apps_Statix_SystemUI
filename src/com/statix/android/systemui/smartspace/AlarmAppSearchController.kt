...
class AlarmAppSearchController @Inject constructor(
    ...
    suspend fun createSearchSession(context: Context?): Unit = suspendCancellableCoroutine { continuation ->
        try {
            ...
            appSearchManager.createGlobalSearchSession(mainExecutor) { sessionResult: AppSearchResult<GlobalSearchSession> ->
                if (sessionResult.isSuccess) {
                    future.set(sessionResult.resultValue)
                } else {
                    future.setException(Throwable("Failed to create session: ${sessionResult.resultCode}"))
                }
            }
            future.addListener({
                try {
    ...
    suspend fun getNextPageSearchResults(searchResults: SearchResults?): List<SearchResult> = suspendCancellableCoroutine { continuation ->
        ...
        try {
            searchResults.getNextPage(mainExecutor, Consumer { result: AppSearchResult<List<SearchResult>> ->
                if (result.isSuccess) {
    ...
    suspend fun query(): SearchResults? = withContext(bgDispatcher) {
