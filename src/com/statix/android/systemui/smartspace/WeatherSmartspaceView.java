...
public class WeatherSmartspaceView extends LinearLayout implements BcSmartspaceDataPlugin.SmartspaceTargetListener, BcSmartspaceDataPlugin.SmartspaceView {
    ...
    public WeatherSmartspaceView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.WeatherSmartspaceView, 0, 0);
        try {
            mIconSize = ta.getDimensionPixelSize(1, context.getResources().getDimensionPixelSize(R.dimen.enhanced_smartspace_icon_size));
            int iconInset = ta.getDimensionPixelSize(0, context.getResources().getDimensionPixelSize(R.dimen.enhanced_smartspace_icon_inset));
    ...
    protected void onAttachedToWindow() {
        ...
        if (TextUtils.equals(mUiSurface, "lockscreen")) {
            ...
            mIsAodEnabled = Settings.Secure.getIntForUser(resolver, "doze_always_on", 0, getContext().getUserId()) == 1;
        }
    ...
    public void onSmartspaceTargetsUpdated(List<? extends Parcelable> targets) {
        ...
        if (smartspaceTargets.size() > 1 || (smartspaceTargets.isEmpty() && TextUtils.equals(mUiSurface, "dream"))) {
        ...
        boolean hasValidTemplate = BcSmartspaceCardLoggerUtil.containsValidTemplateType(templateData);
        ...
        BcSmartspaceCardLoggingInfo.Builder builder = new BcSmartspaceCardLoggingInfo.Builder()
                ...
                .setUid(-1)
                .setDimensionalInfo(BcSmartspaceCardLoggerUtil.createDimensionalLoggingInfo(templateData));
        ...
        if (hasValidTemplate) {
            ...
            if (subItemInfo.getIcon() != null) {
                Drawable iconDrawable = BcSmartSpaceUtil.getIconDrawableWithCustomSize(
            ...
            CharSequence text = subItemInfo.getText() != null && !SmartspaceUtils.isEmpty(subItemInfo.getText())
                    ? subItemInfo.getText().getText() : "";
            CharSequence contentDescription = subItemInfo.getIcon() != null ? subItemInfo.getIcon().getContentDescription() : "";
            ContentDescriptionUtil.setFormattedContentDescription("WeatherSmartspaceView", mView, text, contentDescription);
            if (subItemInfo.getTapAction() != null && isClickable()) {
                BcSmartspaceDataPlugin.SmartspaceEventNotifier notifier = mDataProvider != null
                        ? event -> mDataProvider.notifySmartspaceEvent(event) : null;
                BcSmartSpaceUtil.setOnClickListener(mView, target, subItemInfo.getTapAction(), notifier, "WeatherSmartspaceView", mLoggingInfo, 0);
            }
        } else {
            ...
            BcSmartspaceDataPlugin.SmartspaceEventNotifier notifier = mDataProvider != null
                    ? event -> mDataProvider.notifySmartspaceEvent(event) : null;
            BcSmartSpaceUtil.setOnClickListener(mView, target, headerAction, notifier, "WeatherSmartspaceView", mLoggingInfo, 0);
        }
        if (mRemoveTextDescent) {
            int paddingBottom = mTextDescentExtraPadding - (int) Math.floor(mView.getPaint().getFontMetrics().descent);
    ...
    public void setDozeAmount(float dozeAmount) {
        ...
        BcSmartspaceCardLogger.log(BcSmartspaceEvent.SMARTSPACE_CARD_SEEN, new BcSmartspaceCardLoggingInfo(builder));
    }
    ...
    private void registerAodObserver() {
        ...
        ContentResolver resolver = getContext().getContentResolver();
        resolver.registerContentObserver(Settings.Secure.getUriFor("doze_always_on"), false, mAodSettingsObserver, -1);
    }
    ...
    private class WeatherSmartspaceViewObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange) {
            boolean isAodEnabled = Settings.Secure.getIntForUser(outer.getContext().getContentResolver(), "doze_always_on", 0, outer.getContext().getUserId()) == 1;
