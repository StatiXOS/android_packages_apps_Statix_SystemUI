...
public class CarouselTemplateCard extends BcSmartspaceCardSecondary {
    ...
    public void resetUi() {
        for (int i = 0; i < getChildCount(); i++) {
            ...
            BcSmartspaceTemplateDataUtils.updateVisibility(column.findViewById(R.id.upper_text), View.GONE);
            BcSmartspaceTemplateDataUtils.updateVisibility(column.findViewById(R.id.icon), View.GONE);
            BcSmartspaceTemplateDataUtils.updateVisibility(column.findViewById(R.id.lower_text), View.GONE);
        }
    }
    ...
    public boolean setSmartspaceActions(SmartspaceTarget target, BcSmartspaceDataPlugin.SmartspaceEventNotifier eventNotifier, BcSmartspaceCardLoggingInfo loggingInfo) {
        ...
        long validItemsCount = carouselItems.stream()
                .filter(item -> item.getImage() != null && item.getLowerText() != null && item.getUpperText() != null)
                .count();
        ...
        if (validItems < 4) {
            ...
            for (int i = 0; i < 4; i++) {
                View column = getChildAt(i);
                BcSmartspaceTemplateDataUtils.updateVisibility(column, i <= (3 - (4 - validItems)) ? View.VISIBLE : View.GONE);
            }
            ConstraintLayout firstColumn = (ConstraintLayout) getChildAt(0);
            ConstraintLayout.LayoutParams params = (ConstraintLayout.LayoutParams) firstColumn.getLayoutParams();
        ...
        if (carouselData.getCarouselAction() != null) {
            BcSmartSpaceUtil.setOnClickListener(this, target, carouselData.getCarouselAction(), eventNotifier, "CarouselTemplateCard", loggingInfo, 0);
        }

        for (CarouselTemplateData.CarouselItem item : carouselItems) {
            if (item.getTapAction() != null) {
                BcSmartSpaceUtil.setOnClickListener(this, target, item.getTapAction(), eventNotifier, "CarouselTemplateCard", loggingInfo, 0);
            }
        }
