...
public class SubImageTemplateCard extends BcSmartspaceCardSecondary {
    ...
    public boolean setSmartspaceActions(SmartspaceTarget target, BcSmartspaceDataPlugin.SmartspaceEventNotifier eventNotifier, BcSmartspaceCardLoggingInfo loggingInfo) {
        ...
        for (int i = 0; i < subImages.size(); i++) {
            ...
            switch (icon.getType()) {
                ...
                case android.graphics.drawable.Icon.TYPE_RESOURCE:
                    keyBuilder.append(icon.getResPackage())
            ...
            OnDrawableLoadedListener listener = new OnDrawableLoadedListener(this, prevTargetId, cacheKey, mIconDrawableCache, i, subImages, frameDurationMillis, imageViewRef);

            if (mIconDrawableCache.containsKey(cacheKey) && mIconDrawableCache.get(cacheKey) != null) {
                listener.onDrawableLoaded(mIconDrawableCache.get(cacheKey));
            } else if (icon.getType() == android.graphics.drawable.Icon.TYPE_URI || icon.getType() == android.graphics.drawable.Icon.TYPE_URI_ADAPTIVE_BITMAP) {
                DrawableWrapper wrapper = new DrawableWrapper(icon.getUri(), contentResolver, mImageHeight, listener);
        ...
        if (tapAction != null) {
            BcSmartSpaceUtil.setOnClickListener(this, target, tapAction, eventNotifier, "SubImageTemplateCard", loggingInfo, 0);
        }
    ...
    public static class OnDrawableLoadedListener implements android.graphics.drawable.Icon.OnDrawableLoadedListener {
        ...
        public OnDrawableLoadedListener(SubImageTemplateCard card, String prevTargetId, String cacheKey, Map<String, Drawable> iconDrawableCache, int index, List<Icon> subImages, int frameDurationMillis, WeakReference<ImageView> imageViewRef) {
            this.card = card;
        ...
        public void onDrawableLoaded(Drawable drawable) {
            ...
            if (iconDrawableCache.size() == subImages.size()) {
                ...
                List<Drawable> drawables = iconDrawableCache.values().stream()
                ...
                if (drawables.isEmpty()) {
                    ...
                    if (card.mImageView != null) {
                        card.mImageView.getLayoutParams().width = ViewGroup.LayoutParams.WRAP_CONTENT;
    ...
    public static class DrawableWrapper {
        ...
        public DrawableWrapper(Uri uri, ContentResolver contentResolver, int heightInPx, OnDrawableLoadedListener listener) {
            mUri = uri;
        ...
        protected DrawableWrapper doInBackground(DrawableWrapper... wrappers) {
            ...
            try {
                ...
                ImageDecoder.decodeDrawable(source, (decoder, info, src) -> {
                    ...
                    float aspectRatio = info.getSize().getHeight() != 0 ? (float) info.getSize().getWidth() / info.getSize().getHeight() : 0;
