...
public class BaseTemplateCard extends ConstraintLayout implements SmartspaceCard {
    ...
    @Override
    public void bindData(SmartspaceTarget target, BcSmartspaceDataPlugin.SmartspaceEventNotifier eventNotifier, BcSmartspaceCardLoggingInfo loggingInfo, boolean shouldShowPageIndicator) {
        mTarget = null;
        mTemplateData = null;
        mFeatureType = 0;
        mLoggingInfo = null;
        ...
        if (mSecondaryCard != null) {
            ...
            BcSmartspaceTemplateDataUtils.updateVisibility(mSecondaryCardPane, mValidSecondaryCard ? View.VISIBLE : View.GONE);
        } else {
            Log.i("SsBaseTemplateCard", "Secondary card is null");
            BcSmartspaceTemplateDataUtils.updateVisibility(mSecondaryCardPane, View.GONE);
        }

        // Handle date view for primary card
        if (mDateView != null && mFeatureType == 1) {
            String tapActionId = UUID.randomUUID().toString();
            TapAction tapAction = new TapAction.Builder(tapActionId)
                    .setIntent(BcSmartSpaceUtil.getOpenCalendarIntent())
                    .build();
            BcSmartSpaceUtil.setOnClickListener(this, mTarget, tapAction, eventNotifier, "SsBaseTemplateCard", mLoggingInfo, 0);
            BcSmartSpaceUtil.setOnClickListener(mDateView, mTarget, tapAction, eventNotifier, "SsBaseTemplateCard", mLoggingInfo, 0);
            BcSmartspaceTemplateDataUtils.updateVisibility(mDateView, View.VISIBLE);
        } else if (mDateView != null) {
            BcSmartspaceTemplateDataUtils.updateVisibility(mDateView, View.GONE);
        }
        ...
        setUpTextView(mSubtitleSupplementalView, mTemplateData.getSubtitleSupplementalItem(), eventNotifier);
        setUpTextView(mSupplementalLineTextView, mTemplateData.getSupplementalLineItem(), eventNotifier);
        ...
        boolean subtitleVisible = mSubtitleTextView.getVisibility() != View.GONE || mSubtitleSupplementalView.getVisibility() != View.GONE;
        BcSmartspaceTemplateDataUtils.updateVisibility(mSubtitleGroup, subtitleVisible ? View.VISIBLE : View.GONE);

        if (mTarget.getFeatureType() == 1 && mSubtitleSupplementalView != null && mSubtitleSupplementalView.getVisibility() == View.VISIBLE) {
            mSubtitleTextView.setEllipsize(null);
        }

        if (mTemplateData.getPrimaryItem() != null && mTemplateData.getPrimaryItem().getTapAction() != null) {
            BcSmartSpaceUtil.setOnClickListener(this, mTarget, mTemplateData.getPrimaryItem().getTapAction(), eventNotifier, "SsBaseTemplateCard", mLoggingInfo, 0);
        }
    ...
    public BcSmartspaceCardLoggingInfo getLoggingInfo() {
        ...
        BcSmartspaceCardLoggingInfo.Builder builder = new BcSmartspaceCardLoggingInfo.Builder()
                ...
                .setUid(-1);
        builder.setDimensionalInfo(BcSmartspaceCardLoggerUtil.createDimensionalLoggingInfo(mTarget.getTemplateData()));
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        boolean subtitleTextVisible = mSubtitleTextView != null && mSubtitleTextView.getVisibility() == View.VISIBLE;
        boolean subtitleSupplementalVisible = mSubtitleSupplementalView != null && mSubtitleSupplementalView.getVisibility() == View.VISIBLE;
        if (!subtitleTextVisible && !subtitleSupplementalVisible) {
            return;
        }
        int padding = getResources().getDimensionPixelSize(R.dimen.subtitle_hit_rect_height) - mSubtitleGroup.getHeight();
        ...
        if (subtitleSupplementalVisible) {
            ...
            composite.mDelegates.add(new TouchDelegate(mSubtitleSupplementalHitRect, mSubtitleSupplementalView));
        }
    }
    ...
    public void setDozeAmount(float dozeAmount) {
        ...
        if (mSecondaryCardPane != null && mSecondaryCardPane.getVisibility() == View.VISIBLE && mTextGroup != null) {
    ...
    public void setUpTextView(DoubleShadowTextView textView, BaseTemplateData.SubItemInfo subItemInfo, BcSmartspaceDataPlugin.SmartspaceEventNotifier eventNotifier) {
        if (textView == null) {
        ...
        if (icon != null) {
            ...
            int iconSize = getResources().getDimensionPixelSize(R.dimen.enhanced_smartspace_icon_size);
            Drawable iconDrawable = BcSmartSpaceUtil.getIconDrawableWithCustomSize(icon.getIcon(), getContext(), iconSize);
            drawable.setIcon(iconDrawable);
            textView.setCompoundDrawablesRelative(drawable, null, null, null);
            CharSequence contentDescription = SmartspaceUtils.isEmpty(text) ? "" : text.getText();
            ContentDescriptionUtil.setFormattedContentDescription("SsBaseTemplateCard", textView, contentDescription, icon.getContentDescription());
        ...
        int subCardRank = 0;
        if (mLoggingInfo != null && mLoggingInfo.mSubcardInfo != null && !mLoggingInfo.mSubcardInfo.mSubcards.isEmpty() && subItemInfo.getLoggingInfo() != null) {
            ...
            if (targetFeatureType != mLoggingInfo.mFeatureType) {
                for (int i = 0; i < mLoggingInfo.mSubcardInfo.mSubcards.size(); i++) {
                    ...
                    if (subCard.mInstanceId == subItemInfo.getLoggingInfo().getInstanceId() && subCard.mCardTypeId == targetFeatureType) {
                        subCardRank = i + 1;
                        break;
                    }
                }
