...
public abstract class BcSmartSpaceUtil {
    ...
    public static int getFeatureType(SmartspaceTarget target) {
        ...
        if (actionChips != null && !actionChips.isEmpty() && featureType == 13 && actionChips.size() == 1) {
            return -2;
        }
    ...
    public static Intent getOpenCalendarIntent() {
        return new Intent("android.intent.action.VIEW")
            .setData(ContentUris.appendId(CalendarContract.CONTENT_URI.buildUpon()
            .appendPath("time"),
            System.currentTimeMillis())
            .build()).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);
    }
    ...
    public static void setOnClickListener(View view, SmartspaceTarget target, SmartspaceAction action, BcSmartspaceDataPlugin.SmartspaceEventNotifier eventNotifier, String tag, BcSmartspaceCardLoggingInfo loggingInfo, int index) {
        ...
        boolean shouldShowOnLockscreen = action.getExtras() != null && action.getExtras().getBoolean("show_on_lockscreen");
        boolean noIntent = action.getIntent() == null && action.getPendingIntent() == null;
        BcSmartspaceDataPlugin.IntentStarter intentStarter = sIntentStarter != null ? sIntentStarter : new DefaultIntentStarter(tag);
    ...
    public static class InteractionHandler implements RemoteViews.InteractionHandler {
        ...
        public InteractionHandler(BcSmartspaceCardLoggingInfo loggingInfo, BcSmartspaceDataPlugin.SmartspaceEventNotifier eventNotifier, SmartspaceTarget target, SmartspaceAction action) {
        ...
        @Override
        public boolean onInteraction(View view, PendingIntent pendingIntent, RemoteViews.RemoteResponse response) {
            BcSmartspaceDataPlugin.IntentStarter intentStarter = sIntentStarter != null ? sIntentStarter : new DefaultIntentStarter("BcSmartspaceRemoteViewsCard");
