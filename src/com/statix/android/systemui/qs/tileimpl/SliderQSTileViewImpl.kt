...
class SliderQSTileViewImpl constructor(context: Context,
    ...
    override fun onPercentUpdated(newPercent: Int) = updatePercent(newPercent)

    fun updatePercentBackground(active: Boolean) {
        percentageDrawable.setTint(if (active) { Color.WHITE } else { Color.BLACK })
        val colorBackgroundDrawable = (background as LayerDrawable).findDrawableByLayerId(R.id.background);
        val layerDrawable = LayerDrawable(arrayOf(colorBackgroundDrawable, percentageDrawable)) 
        background = layerDrawable
    }

    private fun updatePercent(newPercent: Int) {
    ...
    private inner class PercentageDrawable : Drawable() {
        ...
        override fun draw(canvas: Canvas) {
            ...
            android.util.Log.d("SliderQSTileViewImpl", (shape.getBounds().width().toFloat() * currentPercent).toString());
            if (width == 0 || shape.getBounds().height() == 0) {
                return
            }
            val bmp = Bitmap.createBitmap((shape.getBounds().width().toFloat() * currentPercent).roundToInt(), shape.getBounds().height(), Bitmap.Config.ARGB_8888)
